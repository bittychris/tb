<div>
                        ...
                        <div class="row justify-content-between align-items-center">
                            ...
                            <div class="col-5">
                                <input type="text" wire:model.live="keywords" class="form-control form-control-sm"
                                    id="keywords" placeholder="Search by attribute name">
                            </div>

                            @if (auth()->user()->can('add attribute'))
                                <div class="col-3">
                                    <button type="button" class="btn btn-primary btn-sm text-white text-uppercase"
                                        data-bs-toggle="modal" data-bs-target="#attribute_form_modal"
                                        style="float:  right;"><span class="me-2" style="font-size: 18px;">+</span>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    @if (auth()->user()->can('edit attribute') ||
                                            auth()->user()->can('delete attribute'))
                                        <th>Action</th>
                                    @endif
                                            ...
                                            <td class="text-center">
                                                ...
                                                @if (auth()->user()->can('edit attribute'))
                                                    <button class="btn btn-warning btn-sm text-white"
                                                        wire:click="prepareData('{{ $attribute->id }}', 'edit')"
                                                        title="Edit"><i class="mdi mdi-pen"></i></button>
                                                @endif

                                                @if (auth()->user()->can('edit attribute'))
                                                    <button class="btn btn-danger btn-sm text-white"
                                                        wire:click="prepareData('{{ $attribute->id }}', 'delete')"
    ...
    <div wire:ignore.self class="modal fade" id="attribute_form_modal" tabindex="-1"
        aria-labelledby="attribute_form_modal_label" aria-hidden="true">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form class="forms-sample"
                    wire:submit.prevent="{{ $editMode == true ? 'updateAttribute' : 'saveAttribute' }}">
                    <div class="modal-header">
                        <h1 class="modal-title fs-5" id="exampleModalLabel">
                            {{ $editMode == true ? 'Edit Attribute' : 'Add Attribute' }}</h1>
                    ...
                    <div class="modal-body px-3">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" wire:model="attribute_no" class="form-control form-control-sm"
                                id="attribute_no" step="0.1" min="0.1"
                                placeholder="Specify Attribute position (number)">
                            @error('attribute_no')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
    ...
    <div wire:ignore.self class="modal fade" id="delete_attribute_modal" tabindex="-1"
        aria-labelledby="delete_attribute_modal_label" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
