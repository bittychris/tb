                    ...
                    <h4 class="card-title">
                        <div class="row">
                            ...
                            @if (auth()->user()->can('add field data'))
                                <div class="col-4">
                                    <a href="{{ route('admin.create_form_data') }}" class="btn btn-primary text-white btn-sm" style="float: right;"><span class="me-2" style="font-size: 18px;">+</span> Add Field data</a>                            
                                </div>
                            @endif
                        </div>
                        <div class="row justify-content-between align-items-center mt-4">
                            <div class="col-5">
                                <input type="text" wire:model.live="keywords" class="form-control form-control-sm" id="keywords" placeholder="Search by form name, ward, or Reginal coordinator names">
                            </div>
                            <div class="col-3">
                            ...
                            <div class="col-4">
                                <a href="{{ route('formattribute.export') }}" class="btn btn-danger text-white btn-sm mb-0" style="float: right;">
                        ...
                        <table class="table table-hover table-bordered table-sm">
                            ...
                            @forelse($reports as $key => $report)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge rounded bg-{{ $report->status == 0 ? 'danger' : 'success' }}">
                                        ...
                                        <td>
                                            ...
                                            @endif
                                             @if (auth()->user()->can('submit field data'))
                                                 <button type="button" class="btn btn-{{ $report->status == 0 ? 'danger' : 'success' }} btn-xs text-white" wire:click="getFormData('{{ $report->id }}')" {{ $report->status == 0 ? '' : 'disabled' }}><i class="mdi mdi-{{ $report->status == 0 ? 'send' : 'check' }}"></i></button>
                                            @endif 
                                        </td>
                                    @endif
