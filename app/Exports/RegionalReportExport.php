    ...
    public function view(): view
    {
        ...
        if ($this->region_id) {
            ...
            $forms = Form::select('forms.*', 'form_attributes.updated_at as upt_at', 'form_attributes.created_at as crt_at')
                ...
                ->whereIn('created_by', array_unique($rc_ids))
            ...
            $firstForm = Form::whereIn('created_by', array_unique($rc_ids))
                // whereIn('ward_id', $ward_ids)
            ...
            $lastForm = Form::whereIn('created_by', array_unique($rc_ids))
                // whereIn('ward_id', $ward_ids)
                ->whereBetween('updated_at', [Carbon::parse($this->startDate)->startOfDay(), Carbon::parse($this->endDate)->endOfDay()])
                ->where('status', true)
                ->latest()
        ...
        foreach ($this->form_ids as $form_id) {
            ...
            if (!empty($form_id)) {
                ...
                $formData = DB::select("SELECT form_data.attribute_id, form_data.age_group_id, 
                        ...
                        JOIN users ON forms.created_by = users.id 
                        -- LEFT JOIN wards ON forms.ward_id = wards.id 
                        -- LEFT JOIN districts ON wards.district_id = districts.id 
                        -- LEFT JOIN regions AS _dist_region ON districts.region_id = _dist_region.id 
